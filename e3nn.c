...
void tensor_product_v4(const Irreps* irreps_1, const float* data_1, const Irreps* irreps_2, const float* data_2, const Irreps* irreps_o, float* data_o) {
    // this is the same as tensor_product above, except the tensor products for
    // any l1,l2,lo are replace with a call to a precompiled version in tp_v2.c
        ...
        for (int i2 = 0; i2 < irreps_2->size; i2++) {
            ...
            for (int lo = lo_min; lo <= lo_max; lo++) {
                ...
                for (int c1_idx = 0; c1_idx < c1; c1_idx++) {
                    ...
                    for (int c2_idx = 0; c2_idx < c2; c2_idx++) {
                        int idx2 = ptr2 + c2_idx * l2_dim;
                        tp_v2(l1, l2, lo, data_1 + idx1, data_2 + idx2, data_o + out_ptr);
                        // done writing to this irrep
                        out_ptr += (2 * lo + 1);
                    }
                }
